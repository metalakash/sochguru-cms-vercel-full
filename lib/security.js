...
/**
 * True only when the gate is switched ON *and* this request carried a valid
 * cookie.
 *
 * isAuthorized() alone is deliberately not enough here: it returns true for
 * everybody when no CMS_ACCESS_CODE is set, so using it to release the server
 * key would hand that key to every anonymous caller on an ungated instance —
 * the exact open endpoint this app already shipped once.
 */
export function isGatedMember(request) {
  return gateEnabled() && isAuthorized(request)
}

/**
 * The operator's own key. Only ever released through resolveGeminiKey().
 *
 * Named SOCHGURUCMSMAIN rather than GEMINI_API_KEY at the operator's request.
 * This is the ONLY name read: no aliases, no fallback chain. A second accepted
 * name is how GEMINI_API_KEY_CMS sat live and forgotten on a public URL for 19
 * days — one name means one place to look when auditing what can spend money.
 */
function serverGeminiKey() {
  return process.env.SOCHGURUCMSMAIN || ''
}

/** Whether this caller can be covered by the operator's key. */
export function serverKeyAvailable(request) {
  return isGatedMember(request) && serverGeminiKey().length > 0
}

/**
 * The one place that decides whose key a request spends: its own first, the
 * operator's only from behind the gate, nothing otherwise. Routes must not read
 * process.env for a key themselves — this chokepoint is what keeps an
 * unauthenticated route from ever becoming billable.
 */
export function resolveGeminiKey(request) {
  const own = userApiKey(request)
  if (own) return { key: own, source: 'user' }
  if (isGatedMember(request)) {
    const server = serverGeminiKey()
